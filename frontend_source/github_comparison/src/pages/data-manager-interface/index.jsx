...
const ManagerDashboard = () => {
  ...
  useEffect(() => {
    ...
    // בדיקה שהאימות לא פג תוקף (24 שעות)
  ...
  // פונקציה בטוחה לפענוח ה-token שמתמודדת עם עברית
  const decodeAuthToken = (token) => {
    try {
      return JSON.parse(decodeURIComponent(atob(token)));
    } catch (e) {
      // ניסיון תמיכה בגרסאות ישנות ללא קידוד
      try {
  ...
  const renderTabs = () => {
    const tabs = [
      ...
      { id: 'orders', label: 'הזמנות (KDS)', icon: '👨‍🍳' },
    ];
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id
  ...
  const statusBanner =
    (statusMessage || errorMessage) && (
      <div
        className={`p-3 rounded-lg text-sm ${errorMessage ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
          }`}
      >
    ...
    <div className="h-screen flex flex-col bg-gray-100" dir="rtl">
      <header className="bg-blue-600 text-white p-2 shadow-md shrink-0 z-20 relative">
        <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
          {/* Right Side: Logout & Status (Visual Right in RTL) */}
