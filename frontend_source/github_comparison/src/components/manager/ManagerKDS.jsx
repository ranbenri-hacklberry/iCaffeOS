...
const ManagerOrderCard = ({ order, isReady = false, onOrderStatusUpdate, onPaymentCollected, onEditOrder }) => {
    ...
    // If we have items in both categories, we force 2 columns.
    // If only one, we can still use the grid but one side will be empty.
    const hasDrinks = drinkItems.length > 0;
    const hasFood = foodItems.length > 0;

    // For visual consistency, if we have > 3 items total OR mixed types, use 2 cols
    // User requested "always drinks right, always food left".
    // So we use a 2-col grid regardless, unless empty.
...
const ManagerKDS = () => {
    ...
    const fetchKDS = async () => {
        ...
        try {
            ...
            // Should use getSupabase(currentUser) but fallback to supabase is safer if context is slow
            const client = supabase; // Force default client to avoid auth issues if user prop is missing

            const { data, error } = await client.rpc('get_kds_orders', { p_date: today.toISOString() });
            if (error) throw error;

            const processed = (data || []).map(order => {
                const rawItems = (order.order_items || []).filter(item =>
                    item.item_status !== 'cancelled' && item.item_status !== 'completed' && item.menu_items?.name
                );

                // Unpaid Completed Handling
                if (order.order_status === 'completed' && !order.is_paid) {
                    const displayItems = groupItems((order.order_items || []).filter(i => i.item_status !== 'cancelled').map(formatItem));
                    return formatOrder(order, displayItems, 'completed', false, 'unpaid_delivered');
                }

                if (rawItems.length === 0) return null;

                const formattedItems = groupItems(rawItems.map(formatItem));
                const hasActive = rawItems.some(i => ['new', 'in_progress', 'pending'].includes(i.item_status));
                const allReady = rawItems.every(i => i.item_status === 'ready');

                let groupStatus = 'prep';
                if (allReady) groupStatus = 'ready';

                return formatOrder(order, formattedItems, groupStatus, order.is_paid);
            }).filter(Boolean);
        ...
        <div className="h-full flex flex-col bg-gray-100">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {loading && orders.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-20 items-stretch">
