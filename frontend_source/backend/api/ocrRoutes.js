...
// Helper to preprocess the image (grayscale L conversion and optional resize) via Python
...
img.convert('L').save('${imagePath}')
`;
        const cmd = `python3 -c "${pythonScript.replace(/\n/g, ' ').trim()}"`;
        console.log(`🤖 Preprocessing image (resize + grayscale): ${cmd}`);
...
/**
 * Common parsing, Supplier matching, and Ollama reasoning logic
 */
const processOcrText = async (ocrText, ocrTextChunks, businessId, res) => {
...
router.post('/process', async (req, res) => {
    ...
    try {
        ...
        await processOcrText(ocrText, ocrTextChunks, businessId, res);
 ...
 * Handles raw file uploads directly to completely avoid browser memory overflow crashes!
 */
router.post('/process-upload', upload.single('invoice'), async (req, res) => {
    if (!req.file) {
        return res.status(400).json({ success: false, error: 'Missing invoice file upload.' });
    }

    const businessId = req.body.businessId || '22222222-2222-2222-2222-222222222222';
    console.log(`🔄 OCR Upload Request received for business: ${businessId}. Processing file: ${req.file.path}`);
    ...
    try {
        if (isPdf) {
            ...
            try {
                const execPromise = (cmd) => new Promise((resolve, reject) => exec(cmd, (err, stdout) => err ? reject(err) : resolve(stdout)));
                digitalText = await execPromise(`pdftotext -layout "${tempImagePath}" -`);
            } catch (err) {
                console.warn('⚠️ pdftotext extraction failed, falling back to OCR:', err.message);
            }
            ...
            if (digitalText && digitalText.trim().length > 100 && hasHebrew) {
                ...
                const pngPrefix = tempImagePath + '_page';
                const pageFiles = await convertPdfToPngs(tempImagePath, pngPrefix);
                pageFilesToCleanup = pageFiles;
            ...
            ocrText = await runTesseract(tempImagePath);
            console.log(`✅ Extracted raw text length: ${ocrText.length}`);
            ocrTextChunks = splitTextIntoChunks(ocrText, 500);
        }
        ...
        console.error('❌ OCR upload processing error:', err);
        return res.status(500).json({ success: false, error: err.message });
    } finally {
        try {
            await fs.promises.unlink(tempImagePath);
