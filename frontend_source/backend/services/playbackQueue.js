...
export class PlaybackQueue {
    ...
    /**
     * Update queue order and current index without resetting playback.
     */
    updateQueue(songs, currentIndex) {
        if (!Array.isArray(songs)) return;
        this._state.queue = songs.map(this._normalizeSong);
        if (typeof currentIndex === 'number') {
            this._state.currentIndex = Math.max(0, Math.min(currentIndex, songs.length - 1));
        }
        this._save();
        console.log(`📋 [PlaybackQueue] Updated queue order: ${songs.length} tracks, current index is #${currentIndex}`);
    }
