...
import { ipcMain as ipcMain$1, app as app$1, net } from "electron";
...
const RAID_MOUNT_POINT = process.platform === "linux" ? "/mnt/raid1" : path$1.join(process.env.HOME || "", "Library/Application Support/icaffeos/data");
const LOG_PATH = process.platform === "linux" ? "/var/log/icaffeos.log" : path$1.join(process.env.HOME || "", "Library/Logs/icaffeos.log");
const BACKUP_DIR = path$1.join(RAID_MOUNT_POINT, "backups");
...
class DiskHealthMonitor {
      ...
      exec(`smartctl -a ${device} --json`, (error, stdout) => {
        ...
        try {
          ...
          resolve({
            ...
            reallocated_sector_ct: ((_c = (_b = (_a = data.ata_smart_attributes) == null ? void 0 : _a.table.find((a) => a.id === 5)) == null ? void 0 : _b.raw) == null ? void 0 : _c.value) || 0,
            power_on_hours: ((_d = data.power_on_time) == null ? void 0 : _d.hours) || 0
          });
        } catch {
          resolve({});
        }
  ...
  evaluateHealth(smart) {
    var _a;
    if (((_a = smart.temperature) == null ? void 0 : _a.current) > 55)
      return "warning";
    if (smart.reallocated_sector_ct > 10)
      return "critical";
    if (smart.reallocated_sector_ct > 0)
      return "warning";
    return "healthy";
  }
}
class BackupService {
  async createBackup(dexieJson, postgresSql) {
    await ensureDir(BACKUP_DIR);
    const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
    const backupName = `backup-${timestamp}`;
    const walFile = path$1.join(BACKUP_DIR, `${backupName}.wal`);
    const tempFile = path$1.join(BACKUP_DIR, `${backupName}.tmp`);
    const finalFile = path$1.join(BACKUP_DIR, `${backupName}.tar.gz`);
    try {
      await fsPromises.writeFile(walFile, "STARTED", "utf8");
      const output = fs$1.createWriteStream(tempFile);
      ...
      if (fs$1.existsSync(walFile))
        await fsPromises.writeFile(walFile, `FAILED: ${error.message}`);
      throw error;
    }
  }
}
class LogStreamer {
  constructor(window2, logPath = LOG_PATH) {
  ...
  startStreaming() {
    if (!fs$1.existsSync(this.logPath)) {
    ...
    try {
      let fileSize = 0;
      try {
        fileSize = fs$1.statSync(this.logPath).size;
      } catch {
      }
      this.watcher = fs$1.watch(this.logPath, (eventType) => {
        if (eventType === "change") {
          try {
            const newStat = fs$1.statSync(this.logPath);
            const newSize = newStat.size;
            if (newSize > fileSize) {
              const stream = fs$1.createReadStream(this.logPath, {
                start: fileSize,
                end: newSize
              });
...
function initializeHardwareModule(mainWindow2) {
  const monitor = new DiskHealthMonitor(mainWindow2);
  const backupService = new BackupService();
  const logger = new LogStreamer(mainWindow2);
  ipcMain$1.handle("storage:get-disk-status", async () => {
    return { free: 100, total: 1e3, usage: 10 };
  });
  ipcMain$1.handle("storage:create-backup", async (_, { dexie, postgres }) => {
    try {
      console.log("Starting backup...");
      const path2 = await backupService.createBackup(dexie, postgres);
      console.log("Backup created at:", path2);
...
class DisplayWatchdog {
  constructor(window2) {
    ...
    __publicField(this, "checkInterval", null);
    __publicField(this, "recoveryCount", 0);
    this.window = window2;
  }
  ...
  recover() {
    ...
    this.window.show();
    this.window.focus();
    ipcMain$1.emit("hardware:request-power-save", null, true);
  }
}
class RendererMonitor {
  constructor(window2) {
    __publicField(this, "window");
    __publicField(this, "lastPingTime", Date.now());
    __publicField(this, "checkInterval", null);
    __publicField(this, "maxMissedPings", 5);
    // More grace for slow starts
    __publicField(this, "pingTimeout", 6e4);
    // 60s interval
    __publicField(this, "restartCooldown", 0);
    this.window = window2;
  }
  start() {
  ...
  triggerReload() {
    const now = Date.now();
    if (now - this.restartCooldown < 6e4 * 2) {
...
class HardwareResilience {
  constructor() {
    __publicField(this, "renderCrashTimes", []);
    __publicField(this, "crashTimes", []);
    __publicField(this, "windowLimit", 6e4 * 10);
    // 10 minutes
    __publicField(this, "maxCrashes", 3);
    app$1.on("child-process-gone", (event, details) => {
      try {
        if (details.type === "GPU") {
    ...
    app$1.on("render-process-gone", (event, webContents, details) => {
      try {
      ...
      if (details.reason !== "clean-exit" && details.reason !== "killed") {
        const now = Date.now();
        this.renderCrashTimes = this.renderCrashTimes.filter((t) => now - t < this.windowLimit);
  ...
  handleGPUCrash(reason) {
    try {
      console.error("💥 GPU Process Crash:", reason);
    } catch (e) {
    }
    const now = Date.now();
    this.crashTimes = this.crashTimes.filter((t) => now - t < this.windowLimit);
...
function initializeWatchdogs(window2) {
  const display = new DisplayWatchdog(window2);
  const renderer = new RendererMonitor(window2);
...
const mpegParserLoader = {
  ...
  async load() {
    return (await import("./MpegParser-DA_wT9UB.js")).MpegParser;
  }
};
...
const asfParserLoader = {
  ...
  async load() {
    return (await import("./AsfParser-D0THw-BV.js")).AsfParser;
  }
};
const dsdiffParserLoader = {
  ...
  async load() {
    return (await import("./DsdiffParser-D47_e9Dg.js")).DsdiffParser;
  }
};
const aiffParserLoader = {
  ...
  async load() {
    return (await import("./AiffParser-mWhuofv1.js")).AIFFParser;
  }
};
const dsfParserLoader = {
  ...
  mimeTypes: ["audio/dsf"],
  async load() {
    return (await import("./DsfParser-D5XqhwTm.js")).DsfParser;
  }
};
const flacParserLoader = {
  ...
  async load() {
    return (await import("./FlacParser-DzDLmNFP.js").then((n) => n.d)).FlacParser;
  }
};
const matroskaParserLoader = {
  ...
  async load() {
    return (await import("./MatroskaParser-l9uF-9Q0.js")).MatroskaParser;
  }
};
const mp4ParserLoader = {
  ...
  async load() {
    return (await import("./MP4Parser-cWdn1CtI.js")).MP4Parser;
  }
};
const musepackParserLoader = {
  ...
  async load() {
    return (await import("./MusepackParser-BNchjbck.js")).MusepackParser;
  }
};
const oggParserLoader = {
  ...
  async load() {
    return (await import("./OggParser-CLVsdLCp.js")).OggParser;
  }
};
const wavpackParserLoader = {
  ...
  async load() {
    return (await import("./WavPackParser-BBUzf3ZT.js")).WavPackParser;
  }
};
const riffParserLoader = {
  ...
  async load() {
    return (await import("./WaveParser-CAQRYJud.js")).WaveParser;
  }
};
...
class DiscoveryService {
  constructor() {
    ...
    this.cacheDir = path$1.join(app$1.getPath("userData"), "cover_cache");
    if (!fs$1.existsSync(this.cacheDir)) {
      fs$1.mkdirSync(this.cacheDir, { recursive: true });
    }
  }
  async scanDirectory(dirPath) {
    ...
    for (const file of files) {
      try {
        ...
        const filename = path$1.basename(file, path$1.extname(file));
        const title = metadata.common.title || filename;
        const artist = metadata.common.artist || "Unknown Artist";
        const album = metadata.common.album || "Unknown Album";
        const hash = require$$1.createHash("md5").update(`${title}-${artist}-${metadata.format.duration}`).digest("hex");
        results.push({
          filePath: file,
          title,
          artist,
          album,
          genre: ((_a = metadata.common.genre) == null ? void 0 : _a[0]) || "Uncategorized",
        ...
        console.warn(`Failed to parse ${file}:`, (err == null ? void 0 : err.message) || err);
      }
    }
    return results;
  }
  async getFilesRecursively(dir) {
    const dirents = await fs$1.promises.readdir(dir, { withFileTypes: true });
  ...
  async cacheCoverArt(metadata, filePath) {
    var _a;
    const picture = (_a = metadata.common.picture) == null ? void 0 : _a[0];
    if (!picture)
      return void 0;
    const hash = require$$1.createHash("md5").update(filePath).digest("hex");
    const fileName = `${hash}.jpg`;
    const destPath = path$1.join(this.cacheDir, fileName);
    if (!fs$1.existsSync(destPath)) {
      await fs$1.promises.writeFile(destPath, picture.data);
    }
...
const handleError$1 = async (error, reject, options, namespace) => {
  if (error && typeof error === "object" && "status" in error && "ok" in error && typeof error.status === "number" && !(options === null || options === void 0 ? void 0 : options.noResolveJson)) {
    ...
    reject(new StorageUnknownError(_getErrorMessage$1(error), error, namespace));
};
  ...
  async uploadOrUpdate(method, path2, fileBody, fileOptions) {
    ...
    return _this.handleOperation(async () => {
      ...
      if (fileOptions === null || fileOptions === void 0 ? void 0 : fileOptions.headers)
        headers = _objectSpread2$1(_objectSpread2$1({}, headers), fileOptions.headers);
...
const DEFAULT_HEADERS$1 = { "X-Client-Info": `gotrue-js/${version$3}` };
...
class GoTrueClient {
  ...
  _logPrefix() {
    return `GoTrueClient@${this.storageKey}:${this.instanceID} (${version$3}) ${(/* @__PURE__ */ new Date()).toISOString()}`;
  }
  _debug(...args) {
    if (this.logDebugMessages) {
...
const MAX_DAILY_UNITS = 9e3;
const SEARCH_COST = 100;
const PLAYLIST_COST = 1;
class YouTubeService {
  constructor() {
    __publicField(this, "apiKey", "");
    __publicField(this, "supabase", null);
    __publicField(this, "quotaPath");
    __publicField(this, "businessId");
    this.quotaPath = path$1.join(app$1.getPath("userData"), QUOTA_FILE);
  ...
  async fetchApiKey() {
    if (!this.supabase)
      return;
    try {
      const { data, error } = await this.supabase.from("businesses").select("youtube_api_key").eq("id", this.businessId).single();
      if (data == null ? void 0 : data.youtube_api_key) {
  ...
  setupRealtimeWatcher() {
    ...
    this.supabase.channel("public:businesses").on(
      "postgres_changes",
  ...
  getQuota() {
    ...
    let quota = { date: today, unitsUsed: 0 };
    if (fs$1.existsSync(this.quotaPath)) {
      try {
        const data = JSON.parse(fs$1.readFileSync(this.quotaPath, "utf-8"));
        if (data.date === today) {
          quota = data;
        }
      } catch (e) {
  ...
  updateQuota(cost) {
    ...
    fs$1.writeFileSync(this.quotaPath, JSON.stringify(quota));
  ...
  async search(query) {
    ...
    if (this.supabase) {
      try {
        const { data: songs } = await this.supabase.from("music_songs").select("id, title, artist:artist_id(name), album:album_id(cover_url)").ilike("title", `%${query}%`).limit(10);
        if (songs) {
          localResults = songs.map((s) => {
            ...
            return {
              id: s.id,
              title: s.title,
              artist: (_a = s.artist) == null ? void 0 : _a.name,
              thumbnail: ((_b = s.album) == null ? void 0 : _b.cover_url) || "",
              isLocal: true,
              source: "LOCAL"
            };
          });
    ...
    try {
      ...
      const youtubeResults = data.items.map((item) => {
        ...
        return {
          ...
          thumbnail: ((_a = item.snippet.thumbnails.high) == null ? void 0 : _a.url) || ((_b = item.snippet.thumbnails.default) == null ? void 0 : _b.url),
          isLocal: false,
          source: "YOUTUBE"
        };
      });
  ...
  async testApiKey(key) {
    ...
    try {
      ...
      const data = await response.json();
      if (data.error) {
        const reason = (_b = (_a = data.error.errors) == null ? void 0 : _a[0]) == null ? void 0 : _b.reason;
...
const { app, BrowserWindow, ipcMain, powerSaveBlocker, globalShortcut } = electron;
...
const PG_PORT = 5432;
const RAID_MOUNT = process.platform === "linux" ? "/mnt/raid1" : path$1.join(app.getPath("userData"), "data");
...
let appStatus = "OPTIMAL";
let failureReasons = [];
let powerSaveId = null;
function togglePowerSave(enable) {
...
function getYtDlpPath() {
  ...
  const resourcePath = path$1.join(process.resourcesPath, "binaries", binaryName);
  if (fs$1.existsSync(resourcePath)) {
...
function setupIPC() {
  ipcMain.handle("auth:get-machine-id", async () => {
    try {
      return await distExports.machineId();
    } catch (e) {
  ...
  ipcMain.handle("music:download-youtube", async (_, { url, artist, album, title }) => {
    return new Promise((resolve, reject) => {
      ...
      let basePath = process.platform === "darwin" ? "/Volumes/Ran1/Music" : "/mnt/music_ssd";
      if (!fs$1.existsSync(basePath)) {
        basePath = path$1.join(require$$0$1.homedir(), "Music", "iCaffe", "Staging");
        console.warn(`⚠️ [Electron] Primary path not found. Falling back to staging: ${basePath}`);
      }
      const folderName = `${safeArtist} - ${safeAlbum}`;
      const dirPath = path$1.join(basePath, folderName);
      const outputPath = path$1.join(dirPath, `${safeTitle}.%(ext)s`);
      try {
        if (!fs$1.existsSync(dirPath)) {
  ...
  if (fs$1.existsSync(volumesPath)) {
    fs$1.watch(volumesPath, (eventType, filename) => {
      if (filename) {
        console.log(`📦 Drive Volume change detected: ${filename} (${eventType})`);
        const isMounted = fs$1.existsSync(path$1.join(volumesPath, filename));
        mainWindow == null ? void 0 : mainWindow.webContents.send("system:volume-change", {
          filename,
          eventType,
          isMounted
        });
        ...
        if (cdNames.some((name) => filename.includes(name))) {
          ...
          mainWindow == null ? void 0 : mainWindow.webContents.send("system:cd-event", {
            isMounted,
            filename
          });
        }
...
async function checkServiceDependencies(updateStatus) {
  let isDegraded = false;
  failureReasons = [];
  updateStatus("Checking Storage...", 10);
  if (!fs$1.existsSync(RAID_MOUNT)) {
    console.warn(`⚠️ Storage path ${RAID_MOUNT} not found. Creating...`);
    try {
      fs$1.mkdirSync(RAID_MOUNT, { recursive: true });
    } catch (e) {
      isDegraded = true;
  ...
  if (!apiRunning) {
    ...
    path$1.join(appPath, "..", "..");
    const pathsToTry = [
      path$1.join(appPath, "backend_server.js"),
      path$1.join(process.cwd(), "backend_server.js")
    ];
    let found = false;
...
async function createMainWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const appPath = app.getAppPath();
    mainWindow.loadFile(path$1.join(appPath, "build/index.html"));
  }
