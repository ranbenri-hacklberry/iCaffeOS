...
const InventoryItemCard = ({ item, onStockChange = null, onOrderChange = null, onUpdate = null, onDelete = null, draftOrderQty = 0 }) => {
    ...
    const precise = (val) => Math.round(val * 100) / 100;
    ...
    const lowStockThresholdGrams = precise((parseFloat(item.low_stock_threshold_units) || 0) * (wpu || 1));
    const isLowStock = lowStockThresholdGrams > 0 && currentStock <= lowStockThresholdGrams;
                        ...
                        >
                            <div className="p-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <NumberStepper
                                        label="התראת מלאי נמוך (יח׳)"
                                        value={editData.low_stock_threshold_units}
                                        onChange={val => setEditData({ ...editData, low_stock_threshold_units: val })}
                                    />
