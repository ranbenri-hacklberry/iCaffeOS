...
export const useMenuItems = (defaultCategory = 'hot-drinks', businessId = null) => {
    ...
    useEffect(() => {
        if (availableCategories.length > 0) {
            const isCurrentValid = availableCategories.some(c => c.id === activeCategory);

            // If current selection is invalid (e.g. empty category or not loaded yet)
            // or if we have no selection yet (null)
