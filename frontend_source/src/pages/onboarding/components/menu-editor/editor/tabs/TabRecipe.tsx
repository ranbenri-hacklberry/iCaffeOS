        ...
        const fetchInventory = async () => {
            ...
            try {
                let query = supabase
                    ...
                    .select('id, name, unit, cost_per_unit, recipe_step')
                    .order('name')
                    .limit(2000);
                            ...
                            <div className="flex items-center gap-1 flex-none" dir="ltr">
                                <button
                                    onClick={() => {
                                        ...
                                        const stepSize = invItem?.recipe_step || (isLargeUnit ? 0.01 : 10);
                                        const newVal = Math.max(0, current - stepSize);
                                        const updated = [...(localItem.recipe || [])];
                                        updated[idx].quantity = isLargeUnit ? newVal.toFixed(3) : String(Math.round(newVal * 10) / 10);
