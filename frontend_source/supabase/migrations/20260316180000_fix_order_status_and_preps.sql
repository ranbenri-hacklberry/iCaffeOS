-- Fix order status to default to 'in_progress' for everyone as per user request
-- Ensure course_stage is handled correctly
...
-- USER REQUEST: Everything starts as in_progress (including delivery for this business)
...
-- Item status should respect the course_stage. If stage 2, it should be 'held'
...
SET quantity = v_item_qty,
    mods = v_item->'mods',
    item_status = v_item_status, -- Ensure status is updated if stage changed
