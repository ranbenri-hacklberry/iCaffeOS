-- Migration: Drop ALL overloads of submit_order_v3 and recreate clean version with course_stage fix

-- Drop all existing overloads
DROP FUNCTION IF EXISTS public.submit_order_v3(text, text, jsonb, boolean, uuid, text, boolean, numeric, text, boolean, uuid, numeric, jsonb, numeric, integer, boolean, uuid, numeric, uuid, text, text, numeric, text);
DROP FUNCTION IF EXISTS public.submit_order_v3(text, text, jsonb, boolean, uuid, text, boolean, numeric, text, boolean, uuid, numeric, jsonb, numeric, integer, boolean, uuid, numeric, uuid, text, text, numeric, text, jsonb);
DROP FUNCTION IF EXISTS public.submit_order_v3 CASCADE;
...
BEGIN
    ...
    LOOP
        ...
        -- 🔑 KEY FIX: Read item_status from payload; course_stage>=2 forces 'held'
        IF v_item_course_stage >= 2 THEN
            v_item_status := 'held';
        ELSE
            v_item_status := COALESCE(NULLIF(v_item->>'item_status', ''), v_order_status);
        END IF;
    ...
    -- 4. Cancel Items in edit mode
    IF p_edit_mode AND p_cancelled_items IS NOT NULL AND jsonb_array_length(p_cancelled_items) > 0 THEN
...
COMMENT ON FUNCTION public.submit_order_v3 IS
'Creates or updates an order. Supports course_stage from payload (stage 2+ = held for second course). Fixed 2026-03-16.';
