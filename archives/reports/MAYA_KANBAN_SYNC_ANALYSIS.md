...
There are synchronization issues between the **Kanban board** (`/kanban`) and the **KDS screen** (`/kds`).
The KDS has been working perfectly for a long time. The problem is suspected to be **exclusively in the Kanban system**.

### Observed Issues

1. **Status updates from Kanban don't sync properly** - When dragging a card to a new column in Kanban, the status sometimes doesn't update in the database or reverts.
2. **Orders appear in wrong columns** - Orders show up in incorrect status columns after refresh.
3. **Customer data missing** - Customer names/phones sometimes don't load correctly in Kanban (shows "אורח" instead of real name).
...
- **`/hooks/useOrders.js`** - Custom hook for order management (THIS IS LIKELY THE PROBLEM)

### KDS System Uses (WORKING CORRECTLY)

- **`/pages/kds/index.jsx`** - Main KDS screen  
- **`/pages/kds/hooks/useKDSData.js`** - Custom hook (2400+ lines, working)
...
## 📁 FULL FILE: /hooks/useOrders.js (THE SUSPECTED PROBLEM)
    ...
    const ordersByStatus = useMemo(() => {
        ...
        columns.forEach(status => {
            ...
            if (status === 'new') {
                ...
                // 'In Prep' column: ตามลำดับที่ผู้ใช้ย้ายการ์ด (Sort by updated_at ASC)
...
## 📁 FULL FILE: /pages/kanban/index.jsx
...
## 📁 FULL FILE: /components/kanban/KanbanBoard.jsx
...
export function KanbanBoard({
    ...
    const handleDragEnd = async (event) => {
        ...
        const currentStatus = findOrder(orderId)?.order_status || findOrder(orderId)?.orderStatus;
...
**Return the ENTIRE fixed file, not partial snippets.**
