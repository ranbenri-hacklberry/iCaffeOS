...
// List of Order IDs identified as missing transactions
const ORDERS_TO_FIX = [
...
async function fixLoyalty() {
    ...
    for (const orderId of ORDERS_TO_FIX) {
        // 1. Fetch Order Details (Using array destructuring to safer handle 0 or 1 results)
        ...
        if (!orders || orders.length === 0) {
            console.warn(`⚠️ Order ${orderId} NOT FOUND in Cloud DB. Skipping.`);
            continue;
        }
        ...
        // Check if transaction already exists (Manual check to be extra safe)
