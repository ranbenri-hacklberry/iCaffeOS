/**
 * OnboardingWizard — Phase 2
 *
 * Displayed when no cortex_tenant_id exists in localStorage.
 * On completion → POST /api/onboarding → persist tenant_id to Zustand + localStorage.
 *
 * Step 1 — Business Type   (IT Lab / Law Firm / Cafe)
 * Step 2 — Core Entities   (tag input + quick-add chips)
 * Step 3 — Business Name + Tone of Voice
 */

import React, { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { cortexFetchPublic } from "../../lib/api";   // public — no tenant header yet
...
export default function OnboardingWizard() {
  ...
  const handleComplete = async () => {
    ...
    // On success, setTenant triggers App re-render → wizard unmounts, no need to reset.
  };
